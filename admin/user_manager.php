<div class="col">
    <div class="container">
        <h4 class="pt-3 py-3">การจัดการผู้ใช้ทั่วไป</h4>

        <div class="row">
            <?php 
                $sql = $conn->query("SELECT * FROM tb_user WHERE user_role = 4");
                    $i=0;
                ...
                <div class="card border-0 shadow rounded-4">
                    <div class="card-body">
                        ...
                        <div class="text-center">
                           ...
                           ?>
                            <button class="btn btn-sm btn-outline-success" id="btn-app" data-role="<?= $fet->user_role; ?>" data-id="<?= $fet->user_id; ?>">ยืนยัน</button>
                            <?php }elseif($fet->user_status == 1) { ?>
                                <button class="btn btn-sm btn-danger" id="btn-dis" data-role="<?= $fet->user_role; ?>" data-id="<?= $fet->user_id; ?>">ระงับการใช้งานชั่วคราว</button>

                            <?php }else{ ?>
                                <button class="btn btn-sm btn-warning" id="btn-app" data-role="<?= $fet->user_role; ?>" data-id="<?= $fet->user_id; ?>">ยกเลิกการระงับ</button>
